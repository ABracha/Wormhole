...
from utils.geometry_util import get_all_operators, to_basis, from_basis, compute_hks_autoscale, compute_wks_autoscale, data_augmentation, mesh_vertex_normal
...
class LearnedTimeDiffusion(nn.Module):
    ...
    def forward(self, feat, L, mass, evals, evecs):
        ...
        if self.method == 'spectral':
            ...
            mat_dense = L.to_dense().unsqueeze(1).expand(-1, self.in_channels, -1, -1)
    ...
    def forward(self, verts, faces=None, feats=None):
        ...
        if feats is not None:
            ...
            if self.input_type == 'hks':
                ...
                if self.training and self.use_normals:
                    v = verts.cpu().numpy()
                    f = faces.cpu().numpy()
                    normals = igl.per_vertex_normals(v[0],f[0])
                    normals = torch.tensor(normals,dtype=torch.float32,device=verts.device).unsqueeze(0)
                    normals[torch.isnan(normals)] = 0
                    verts, normals = data_augmentation(verts, normals)
                    verts = torch.cat((verts,normals),dim=-1)
                elif self.training:
