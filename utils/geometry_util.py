...
def sparse_np_to_torch_v2(indices,values, dims):
    return torch.sparse_coo_tensor(torch.LongTensor(indices[0]), torch.FloatTensor(values[0]), torch.Size(dims)).coalesce()
...
def sparse_torch_to_np_v2(A):
    assert len(A.shape) == 2

    indices = torch2np(A.indices())
    values = torch2np(A.values())

    return indices, values, A.shape
...
def compute_operators(verts, faces, k=120, normals=None):
    ...
    if is_cloud:
        ...
        # boundry_edges = igl.boundary_facets(faces_np)
        # boundry_vertices = np.unique(boundry_edges.flatten())
        # massvec_np[boundry_vertices] = 0
        # L[boundry_vertices, :] = 0
        # L.eliminate_zeros()
        # L[boundry_vertices, boundry_vertices] = 1
...
def data_augmentation(verts, normals = None, rot_x=0, rot_y=90.0, rot_z=0, std=0.01, noise_clip=0.05, scale_min=0.9, scale_max=1.1):
    # random rotation
    ...
    normals = torch.bmm(normals, rotation_matrix.transpose(1, 2)) if normals is not None else None
...
def furthest_point_sampling_from_distance_matrix(distance_matrix, num_samples):
    """
    Furthest Point Sampling algorithm using a precomputed distance matrix.

    Args:
    - distance_matrix (np.ndarray): The precomputed distance matrix of shape (N, N) where N is the number of points.
    - num_samples (int): The number of points to sample from the input points.

    Returns:
    - np.ndarray: The sampled point indices of shape (num_samples,).
    """
    ...
    for i in range(1, num_samples):
        distances = np.minimum(distances, distance_matrix[sampled_indices[i-1]])
...
def furthest_point_sampling(points, num_samples):
    """
    Simple implementation of the Furthest Point Sampling (FPS) algorithm.

    Args:
    - points (np.ndarray): The input point cloud array of shape (N, D) where N is the number of points
                            and D is the dimensionality of each point.
    - num_samples (int): The number of points to sample from the input point cloud.
...
def calculate_dist_no_boundary_mask(D,faces):
    boundry_edges = igl.boundary_facets(faces)
    boundry_vertices = np.unique(boundry_edges.flatten())
    D_boundry = D[boundry_vertices,:]
    D_dist_to_boundry = np.min(D_boundry,0,keepdims=True)
    mask = np.where(D < D_dist_to_boundry + np.transpose(D_dist_to_boundry) + 1e-7,1,0)
    non_binary_mask = D_dist_to_boundry + np.transpose(D_dist_to_boundry) + 1e-7
    print(np.sum(mask)/mask.shape[0]/mask.shape[1])
    return non_binary_mask

def calculate_minimal_dist_mask(D,verts,faces):
    boundry_edges = igl.boundary_facets(faces)
    boundry_vertex_indices = boundry_edges.flatten()
    D_boundry = D[boundry_vertex_indices,:]
    D_dist_to_boundry = np.min(D_boundry,0,keepdims=True)
    D_dist_to_boundry_indices = np.argmin(D_boundry,0)
    B_B_euc = scipy.spatial.distance_matrix(verts[boundry_vertex_indices[D_dist_to_boundry_indices],:],verts[boundry_vertex_indices[D_dist_to_boundry_indices],:])
    return D_dist_to_boundry + np.transpose(D_dist_to_boundry) + B_B_euc

def calculate_sum_min(a,b,c,init_sample,end_sample,init_sample2,end_sample2):
    if init_sample is None:
        D_boundry_D_boundry_V1_V2_half =  a[:,:end_sample,:,:] + b + c[:,:end_sample,:,:]
        D_boundry_D_boundry_V1_V2_half_min1 = torch.min(D_boundry_D_boundry_V1_V2_half,dim=(0))[0]
        D_boundry_D_boundry_V1_V2_half_min = torch.min(D_boundry_D_boundry_V1_V2_half_min1,dim=(0))[0]
    elif end_sample is None:
        D_boundry_D_boundry_V1_V2_half =  a[:,init_sample:,:,:] + b + c[:,init_sample:,:,:]
        D_boundry_D_boundry_V1_V2_half_min1 = torch.min(D_boundry_D_boundry_V1_V2_half,dim=(0))[0]
        D_boundry_D_boundry_V1_V2_half_min = torch.min(D_boundry_D_boundry_V1_V2_half_min1,dim=(0))[0]
    else:
        D_boundry_D_boundry_V1_V2_half =  a[:,init_sample:end_sample,:,:] + b[init_sample2:end_sample2,:,:,:] + c[init_sample2:end_sample2,init_sample:end_sample,:,:]
        D_boundry_D_boundry_V1_V2_half_min1 = torch.min(D_boundry_D_boundry_V1_V2_half,dim=(0))[0]
        D_boundry_D_boundry_V1_V2_half_min = torch.min(D_boundry_D_boundry_V1_V2_half_min1,dim=(0))[0]
    return D_boundry_D_boundry_V1_V2_half_min


def calculate_advanced_dist_mask_fps(D,verts,faces, fps = False, fps_samples = 0):
    boundry_edges = igl.boundary_facets(faces)
    boundry_vertexs_indices = np.unique(boundry_edges.flatten())
    boundry_samples = len(boundry_vertexs_indices)
    sampled_boundery_indices = boundry_vertexs_indices[:]
    if fps:
        boundry_samples = min(fps_samples,len(boundry_vertexs_indices))
        boundry_vertices = verts[boundry_vertexs_indices,:]
        D_boundry = D[boundry_vertexs_indices,:]
        D_boundry_boundry = D_boundry[:,boundry_vertexs_indices]
        sampled_boundery = furthest_point_sampling_from_distance_matrix(D_boundry_boundry,boundry_samples)
        sampled_boundery_indices = boundry_vertexs_indices[sampled_boundery]
    D_sampled_boundry = D[sampled_boundery_indices,:]
    D_sampled_boundry_euc = scipy.spatial.distance_matrix(verts[sampled_boundery_indices],verts[sampled_boundery_indices])[...,None,None]
    
    # D_dist_to_boundry = np.argmin(D_boundry,0,keepdims=True)
    # D_boundry_euc = scipy.spatial.distance_matrix(verts[boundry_vertexs_indices],verts[boundry_vertexs_indices])[...,None,None]
    # result = None
    V1_D_boundry = np.expand_dims(D_sampled_boundry, 1)
    V1_D_boundry = np.expand_dims(V1_D_boundry, -1)
    V2_D_boundry = np.expand_dims(D_sampled_boundry, 0)
    V2_D_boundry = np.expand_dims(V2_D_boundry, 2)
    print(V2_D_boundry.shape)
    torch.cuda.empty_cache()
    with torch.no_grad():
        V1_D_boundry_cuda = torch.from_numpy(V1_D_boundry).cuda().float()
        V2_D_boundry_cuda = torch.from_numpy(V2_D_boundry).cuda().float()
        D_sampled_boundry_euc_cuda = torch.from_numpy(D_sampled_boundry_euc).cuda().float()
        # D_boundry_D_boundry_V1_V2_half =  V2_D_boundry[:,:boundry_samples//2,:,:] + V1_D_boundry + D_sampled_boundry_euc[:,:boundry_samples//2,:,:]
        # D_boundry_D_boundry_V1_V2_half = np.min(D_boundry_D_boundry_V1_V2_half,axis=(1),keepdims=True)
        # D_boundry_D_boundry_V1_V2_half2 =  V2_D_boundry[:,boundry_samples//2:,:,:] + V1_D_boundry + D_sampled_boundry_euc[:,boundry_samples//2:,:,:]
        # D_boundry_D_boundry_V1_V2_half2 = np.min(D_boundry_D_boundry_V1_V2_half,axis=(1),keepdims=True)
        # D_boundry_D_boundry_V1_V2 = np.minimum(D_boundry_D_boundry_V1_V2_half2,D_boundry_D_boundry_V1_V2_half)
        # max_dist = np.min(D_boundry_D_boundry_V1_V2,axis =(0,1))
        # max_dist = np.minimum(calculate_minimal_dist_mask(D,verts,faces),max_dist)
        # D_boundry_D_boundry_V1_V2_half =  V2_D_boundry_cuda[:,:boundry_samples//2,:,:] + V1_D_boundry_cuda + D_sampled_boundry_euc_cuda[:,:boundry_samples//2,:,:]
        # D_boundry_D_boundry_V1_V2_half_min1 = torch.min(D_boundry_D_boundry_V1_V2_half,dim=(0))[0]
        # D_boundry_D_boundry_V1_V2_half_min = torch.min(D_boundry_D_boundry_V1_V2_half_min1,dim=(0))[0]
        # del D_boundry_D_boundry_V1_V2_half
        # del D_boundry_D_boundry_V1_V2_half_min1
        # sleep(0.1)
        step = 16
        if V1_D_boundry_cuda.shape[2] > 3500:
            step = 8
        if V1_D_boundry_cuda.shape[2] > 6000:
            step = 7
        if V1_D_boundry_cuda.shape[2] > 7000:
            step = 6
        for i in range(0,boundry_samples,step):
            print(i)
            for j in range(0,boundry_samples,step):
                torch.cuda.empty_cache()
                if i == 0 and j == 0:
                    D_boundry_D_boundry_V1_V2 = calculate_sum_min(V2_D_boundry_cuda,V1_D_boundry_cuda,D_sampled_boundry_euc_cuda,i,min(boundry_samples,i+step),j,min(boundry_samples,j+step))
                else:
                    tmp = calculate_sum_min(V2_D_boundry_cuda,V1_D_boundry_cuda,D_sampled_boundry_euc_cuda,i,min(boundry_samples,i+step),j,min(boundry_samples,j+step))
                    D_boundry_D_boundry_V1_V2 = torch.minimum(tmp,D_boundry_D_boundry_V1_V2)
            
        
        # D_boundry_D_boundry_V1_V2_half2 =  V2_D_boundry_cuda[:,boundry_samples//2:,:,:] + V1_D_boundry_cuda + D_sampled_boundry_euc_cuda[:,boundry_samples//2:,:,:]
        # D_boundry_D_boundry_V1_V2_half2 = torch.min(D_boundry_D_boundry_V1_V2_half2,dim=(0))[0]
        # D_boundry_D_boundry_V1_V2_half2 = torch.min(D_boundry_D_boundry_V1_V2_half2,dim=(0))[0]
        # D_boundry_D_boundry_V1_V2 = torch.minimum(D_boundry_D_boundry_V1_V2_half2,D_boundry_D_boundry_V1_V2_half_min)
                

        torch.cuda.empty_cache()
        max_dist = torch.minimum(torch.from_numpy(calculate_minimal_dist_mask(D,verts,faces)).cuda().float(),D_boundry_D_boundry_V1_V2)
        # mask = torch.where(torch.from_numpy(D).cuda().float() < max_dist + 1e-7,1,0).cpu().numpy()
        # print(np.sum(mask)/mask.shape[0]/mask.shape[1])
        return max_dist.cpu().numpy()
    




def calculate_advanced_dist_mask(D,verts,faces):
    boundry_edges = igl.boundary_facets(faces)
    boundry_vertexs_indices = np.unique(boundry_edges.flatten())
    boundry_samples = len(boundry_vertexs_indices)
    sampled_boundery_indices = boundry_vertexs_indices[:]
