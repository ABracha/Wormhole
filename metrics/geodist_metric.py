...
@METRIC_REGISTRY.register()
def calculate_p2p_geodesic_error(dist_x, corr_x, corr_y, p2p,  return_mean=True):
    """
    Calculate the geodesic error between predicted correspondence and gt correspondence
    ...
    corr_y = corr_y[mask]
    p2p = p2p[mask]
    ind21 = np.stack([p2p, corr_y], axis=-1)
    ind21 = np.ravel_multi_index(ind21.T, dims=[dist_x.shape[0], dist_x.shape[0]])
    geo_err = np.take(dist_x, ind21)
    if return_mean:
        return geo_err.mean()
...
@METRIC_REGISTRY.register()
def plot_pck(geo_err, threshold=1.00, steps=100):
    """
